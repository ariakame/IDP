...
def preprocess_audio(file_path, model_type):
    try:
        ...
        if model_type == 'heart':
            ...
            hop_length = max_len // expected_width
...
def predict_sound(file_path, model_type):
    try:
        ...
        # Binary classification (1 neuron output)
        if prediction.shape[1] == 1 and len(classes) == 2:
            raw_value = prediction[0][0]
            confidence = float(raw_value) if np.isscalar(raw_value) else float(raw_value[0])
            ...
            predicted_class_idx = int(np.argmax(prediction_flat))
            confidence = float(prediction_flat[predicted_class_idx])
            class_probs = {classes[i]: float(prediction_flat[i]) for i in range(len(classes))}
